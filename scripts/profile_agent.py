...
def language_bar(repos: list[dict]) -> list[str]:
    """Byte-accurate language mix across public repos, as a unicode bar chart."""
    totals: dict[str, int] = {}
    for repo in repos:
    ...
    for name, size in ranked:
        ...
        rows.append(f"{name:<{pad}}  {bar}  {share * 100:5.1f}%")
    rows.append("```")
    return rows
...
def activity_feed(events: list[dict]) -> list[str]:
    ...
    for _, event in fresh:
        ...
        if not repo or any(
            ...
            continue  # skip the agent's own bookkeeping commits
...
def render() -> str | None:
    ...
    if bar := language_bar(repos):
        blocks.append(["### 🧬 What I actually write", "", *bar])
    ...
    blocks.append([
        ...
        f"{len(repos)} public repos · {stars} stars · last run {stamp}</sub>",
        "",
        "</div>",
    ])
